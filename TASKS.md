...
- **Task 307**: Run `tools/analyst_kit/claims_2026_04_26.py` stale-claim sweep and document findings (2026-04-26, /loop cycle)
  - **Why this cycle**: the analyst's `claims_2026_04_26.py` was committed (commit 2c1920e, "post-bench claim sweep") but had not been executed yet — the output dir held only `00_session_log.md`, `baseline_full.log`, and `quest_topk_probe.md`. The session log scheduled "post-bench claim sweep" at ~02:00 PDT.
  - **Method**: ran the script directly. It does NOT load the model (synthetic tensors at production shapes B=1, H_kv=4, D=128, fp16). Total runtime: ~5 seconds.
  - **Results** (full interpretation in `research/analyst_runs/2026-04-26/claims_interpretation.md`):
    - **F1a (`mx.where(..., 0.0)` vs `mx.where(..., mx.zeros_like(x))`)**: bit-identical, 1.03-5.94× speedup depending on T. Anomaly: T=16K shows only 1.03× (~2.2 ms both ways), while T=4K shows 5.94× and T=64K shows 1.62×. Worth re-running at T=16K with more trials.
    - **F1b (broadcast+take_along_axis vs slice for arange-identity)**: 131-1430× ratio in slice's favor. Cannot be directly leveraged for the mixed-row trim path (Task 295(b) revert lesson).
    - **DuoKV update_and_fetch scaling**: 8K→16K ratio is 0.93× — **decode-step `update_and_fetch` is constant past 8K**. Decode-cliff bottleneck is NOT this path.
    - **StreamingKV ring-mode step**: 0.079 ms median, 50 µs variance. O(1) as designed.
    - **48-layer simulation**: F1a saves 0.26 ms/step at 4K, **2.04 ms/step at 16K** (~3.2% of decode-step time at 62.5 ms/token).
  - **Heap deltas are zero across all measurements** — no leak, no retained allocation. Static-review F1's "leaks at decode-step granularity" hypothesis is FALSIFIED. The pressure is transient peak, not retained.
  - **Deliverables**:
    - `research/analyst_runs/2026-04-26/claims.json` — raw results.
    - `research/analyst_runs/2026-04-26/claims_summary.txt` — script-generated summary.
    - `research/analyst_runs/2026-04-26/claims_interpretation.md` — analyst-style interpretation with recommendations and what's validated/falsified.
  - **Effort**: ~15 min (run + interpretation writeup).
  - **What this enables**: the analyst can now reference concrete numbers for static_review F1/F1b/F3 hypotheses when ranking tasks. F1a is shippable (~3% decode improvement at 16K), F1b doesn't translate to a trivial fix, F3 may not be worth the cycle (the cross-layer cache already amortizes most of the cost).

- **Task 24**: Probe MLX argpartition speed for Quest top-K page selection (2026-04-26, /loop cycle)
  - **Method**: extended existing `scripts/probe_quest_topk.py` to add a Task 24-spec section: `n_heads=32` Q-heads (matches Quest's actual scoring fan-out, vs the script's prior `n_heads=4` KV-heads), `K ∈ {256, 1024, 4096}` (the realistic select-many-pages regime per Quest paper). Also refined the verdict logic: small-ctx FAILs are kernel-launch-overhead artifacts and don't reflect production cost; the production gate only counts ≥1M ctx rows.
  - **Findings** (full writeup in `research/analyst_runs/2026-04-26/quest_topk_probe.md`):
    - **At idle conditions, 1M ctx, K=4096**: 188 µs/head — passes the 200 µs/head gate. `mx.argpartition` is shippable for Task 3 (Quest runtime integration).
    - **Variance is the dominant finding**: same config measured 49 µs/head in the first run and 866 µs/head in the second run of this script. **17× variance on the same workload** — likely Metal GPU contention with concurrent processes (browser/editor/Claude Code), which IS the Hypercar production workload per CLAUDE.md.
    - **At 8M ctx (forward-looking)**: all configs FAIL by 3-7× regardless of run. If Hypercar's 1M target ever extends to 8M+, a custom Metal top-K kernel becomes load-bearing.
  - **Recommendation**: ship Quest using `mx.argpartition` as written. The variance is a Goal 3 "constant across context window" concern (not a "≥50 tok/s" concern — peak tok/s still hits at idle). If observed Goal 3 violation traces to Quest top-K specifically, a custom Metal kernel can close the gap as a follow-up.
  - **Surprising secondary finding**: K=4096 is FASTER than K=256 at 1M ctx (188 vs 267 µs/head). MLX's argpartition kernel has a fixed-cost component dominated by trial setup rather than partition work — choosing the right K is about quality (more pages = better recall), not latency.
  - **Effort**: ~25 min (extending existing script + 2 runs + writeup).
  - **What this de-risks**: Task 3 (Quest runtime integration) can proceed without a custom kernel as a hard prerequisite. The "200 µs/head gate" framing should be revised to "200 µs/head at idle + plan for 10× variance at runtime".
...
### 303. DSA cache-locality diagnostic + LL cache reservation policy — close the 16K→32K decode cliff
- **Goal**: 3 (decode cliff at 16K→32K is currently -43%, the most concrete remaining Goal-3 gap; cache-locality framing is the first one in passes 40-67 that fits the symptoms), Goal 6 (token-granularity LRU keeps the working set small in unified memory, complementary to existing eviction without state inflation)
- **Derived from**: 2603.13430 "Dynamic Sparse Attention: Access Patterns and Architecture" (Noam Levy, March 2026). The paper diagnoses dynamic-sparse-attention decode bottlenecks as cache-locality problems (fragmented, volatile working set; unprefetchable; high LL miss rate) — a description that maps verbatim onto Hypercar's DuoKV gather/mask path at long context. Tasks 269/271 partially implemented this insight at the per-call layer (module-level trim-index cache); 2603.13430's contribution is the missing cross-step coordination layer.
- **Change**: Phase 0 (microbench validation, 1 day): write a standalone microbench in `omlx/bench/dsa_locality_probe.py` that reproduces the paper's per-layer KV-access pattern logger on Hypercar's DuoKV decode loop. Instrument 2K/4K/8K/16K/32K decode and capture: (a) per-step KV index sets per layer, (b) Jaccard overlap between consecutive steps (working-set churn), (c) Metal memory peak resident-set delta. Use `tools/analyst_kit/parse_bench_results.py`-compatible JSON output. Acceptance: working-set churn metric correlates >0.7 with the observed decode tok/s at each context length, and the 16K→32K transition shows a >2× working-set-churn jump matching the -43% decode regression. If correlation <0.5, document the gap and reduce task priority before Phase 1 — the cache-locality framing is wrong and we need a different diagnosis. Phase 1 (1-2 weeks): implement `omlx/duo_kv_locality.py` adding (1) a per-decode-step "hot-token" tracker that maintains a fixed-size set of recently-touched KV indices across all layers, (2) a residency hint API that calls `mx.metal.set_resident()` (or the MLX equivalent — investigate what is exposed) on the hot-token slice before each decode step, (3) a token-granularity LRU eviction policy on the hot set when it exceeds the configured budget, (4) `--dsa-locality-budget K` CLI flag (default K=4096 per layer, off if 0 — preserves backward compat). Phase 2 (optional): integrate with DuoKV's existing trim-index cache so the LL-reservation set is computed from the same working-set signal that drives trim.
- **Verify**: (a) Phase 0 microbench produces a JSON snapshot in `bench/snapshots/devloop-N/dsa_locality.json` with the four metrics above. (b) Correlation analysis in `research/analyst_runs/2026-04-26/dsa_locality_diagnostic.md` documenting whether the cache-locality framing fits Hypercar's symptoms. (c) Phase 1 unit tests in `tests/test_dsa_locality.py` covering hot-token tracker correctness, LRU eviction, and `--dsa-locality-budget=0` no-op fallback. (d) `efficiency_profile --decode-ctx 16384 --kv-mode duo --decode-n 64 --dsa-locality-budget 4096` produces decode tok/s no worse than baseline (regression gate; the policy must not slow decode in the cases it doesn't help). (e) `efficiency_profile --decode-ctx 32768 --kv-mode duo --decode-n 64 --dsa-locality-budget 4096` produces decode tok/s ≥15% above baseline at 32K (target: recover from 10.21 tok/s toward the 13-14 tok/s range that closes part of the cliff); if <5% improvement, mark Phase 1 incomplete and document the gap. (f) `hypercar_bench --full` with `--dsa-locality-budget 4096` passes all 11 gates with no quality regression vs baseline; the policy is residency-only, no algorithmic change. (g) Metal peak memory at 32K decode with `--dsa-locality-budget 4096` does NOT exceed 41.0 GB (the Goal-6 ceiling currently sits at 40.7 GB with 0.5 GB margin; this task is gated on memory neutrality).
- **Effort**: M (1.5-2.5 weeks total — Phase 0 microbench + correlation analysis 1-2 days, Phase 1 implementation + tests 1 week, integration + benchmark validation 3-4 days)
- **Depends on**: Tasks 269 + 271 (module-level trim-index cache) shipped — they are the per-call optimization layer this task builds the cross-step coordination layer on top of. Task 290 (32K efficiency_profile measurement) shipped — provides the baseline number this task improves over. No hard prerequisite on Qwen3.6 migration; the cache-locality diagnosis is model-agnostic and should hold for both Qwen3-Coder and Qwen3.6.

### 304. AFBS-BO automated sparse-attention hyperparameter tuning harness — replace manual grid search across Tasks 246/253/255/256
- **Goal**: 3 (every sparse-attention path in the queue accelerates decode but only when correctly tuned for Hypercar's hardware; manual tuning is the integration tax), Goal 2 (hyperparameter-validated configurations are the only honest path from "paper-reported speedup" to "Hypercar-shipped speedup" — automation removes the per-path human cost)
- **Derived from**: 2603.18417 "Self-Tuning Sparse Attention: Multi-Fidelity Hyperparameter Optimization for Transformer Acceleration" (Arundhathi Dev and Justin Zhan, March 2026). Closes a gap surfaced by the 2026-04-26 microbench-validation feedback memory: any sparse-attention method Hypercar adopts has per-layer/per-head hyperparameters that don't transfer across hardware (Llama-2-7B/CUDA → Qwen3-Coder-30B/Metal). AFBS-BO combines Bayesian Optimization (global) with binary search (local) and multi-fidelity evaluation (short-context first, long-context last) to discover hyperparameters 3.4× faster with 8.8× fewer evaluations than grid search.
- **Change**: Phase 0 (microbench validation, 0.5-1 day): write `omlx/bench/afbs_bo_probe.py` that ports the AFBS-BO optimization loop and runs it on one fixed sparse-attention method (recommended: Twilight, Task 253, since it is the highest-leverage paper from pass 59 and is currently un-tuned for Hypercar's Qwen3-Coder). Acceptance: realized speedup of automated tuning vs grid-search baseline within 30% of paper's 3.4× claim (i.e., ≥2.4× faster discovery on the same search space). If <30% of claim, document the gap in `research/analyst_runs/2026-04-26/afbs_bo_validation.md` and reduce task priority before Phase 1 — paper's claims may not transfer. Phase 1 (1 week): generalize to a parametric harness in `omlx/tuning/afbs_bo.py` that accepts (a) any sparse-attention method via a callable evaluator, (b) per-method hyperparameter spec (name, type, range), (c) a multi-fidelity ladder (short → medium → long context), and produces a per-{model, hardware, method} tuned-config JSON dropped into `bench/snapshots/tuning/<method>_<hardware>.json`. Phase 2 (optional): wire the tuning harness into `hypercar_bench --tune` flag so users can re-tune for their own hardware in one command; cache hit makes the flag a no-op.
- **Verify**: (a) Phase 0 microbench produces `bench/snapshots/devloop-N/afbs_bo_twilight.json` with grid-search-vs-AFBS-BO comparison metrics. (b) Validation memo `research/analyst_runs/2026-04-26/afbs_bo_validation.md` documents whether AFBS-BO's 3.4× claim transfers to Hypercar's hardware + Qwen3-Coder workload. (c) Phase 1 unit tests in `tests/test_afbs_bo.py` covering the optimization loop, parametric evaluator interface, and per-method config serialization. (d) Phase 1 integration test: harness drives Twilight (Task 253) tuning end-to-end and produces a config that, when applied, is within 5% of grid-search-discovered Twilight config quality on `hypercar_bench --quick`. (e) Phase 1 reusability test: harness drives a second sparse-attention method (recommended: SpargeAttn Task 255) without harness code changes — only a new evaluator and hyperparameter spec. (f) Cached configs reproduce: re-running `hypercar_bench --tune` with cache populated is a no-op (≤5 seconds — just the cache-hit check). (g) Quality regression gate: `hypercar_bench --full` with Twilight applied via AFBS-BO-tuned config passes all 11 gates with no quality regression vs Twilight + grid-search-tuned config baseline.
- **Effort**: M (1.5-2 weeks total — Phase 0 microbench + validation memo 1 day, Phase 1 parametric harness 1 week, Twilight + SpargeAttn integration tests 3-4 days)
- **Depends on**: Task 253 (Twilight integration) shipped or in-progress as a Phase 0 driver — if Task 253 hasn't started, AFBS-BO-on-Twilight is impossible to validate. Soft dependency on Tasks 246, 255, 256 — they are the sparse-attention paths the harness generalizes across; Phase 1 reusability test only requires one of them. No model dependency; harness is model-agnostic.

### 313. Theoretical KV-placement upper bound for Apple Silicon tiers — diagnostic ceiling for the 16K→32K decode cliff
- **Goal**: 3 (decode cliff diagnosis — pairs with Task 303's empirical cache-locality measurement to bound how much of the cliff is closeable by placement vs needing pure-Metal-kernel work), Goal 6 (placement annotations are residency-only — they describe *where* tensors should live, not how much state to add)
- **Derived from**: 2508.13231 "Accelerating LLM Inference via Dynamic KV Cache Placement in Heterogeneous Memory System" (Yunhua Fang et al., IEEE Computer Architecture Letter, Aug 2025). The paper formalizes KV cache placement as a knapsack-style optimization: given per-tier bandwidth `B_i` and capacity `C_i`, find the placement assignment that maximizes aggregated bandwidth utilization. Their target hardware is Grace Hopper (HBM + LPDDR5X over NVLink) — discrete address spaces. Apple Silicon is unified, but the *math* doesn't care about address-space sharing; only per-tier bandwidth and capacity matter. Pairs with Task 303's empirical DSA cache-locality diagnostic — Task 303 measures *what's happening*, this task computes *what's achievable*.
- **Change**: Phase 0 (microbench validation, 1-2 days). (1) Instrument Hypercar's bench harness to measure per-tier hit/miss counts at each context length: GPU L2 (~16 MB on M4 Pro), System Level Cache (~24 MB), unified DRAM (48 GB minus model + runtime), swap-paged regions. Use Metal's GPU performance counters where exposed; fall back to `mx.metal.get_active_memory()` + access-frequency heuristics where not. Output JSON snapshot at `bench/snapshots/devloop-N/tier_residency.json`. (2) Port the paper's placement formulation to Hypercar's tier vector: extract per-tier `B_i` (GPU L2 ≈ 5 TB/s, SLC ≈ 800 GB/s, DRAM ≈ 273 GB/s on M4 Pro, swap ≈ 0.43 GB/s) and per-tier `C_i` from Apple's developer references. (3) Solve the placement LP (knapsack relaxation) using either scipy.optimize.linprog or a hand-rolled greedy approximation; output the theoretical upper-bound decode tok/s as a function of context length 2K → 32K. (4) Compare the upper bound vs measured tok/s curve. **Acceptance gate**: if the bound is within 10% of measured tok/s at every context length (i.e., we are *already* near the placement ceiling), file the result as a formalized dead end — placement work is dead-weight, the bottleneck is purely algorithmic. If the bound is >2× measured at any context length (placement headroom exists), proceed to Phase 1. Phase 1 (1-2 weeks, conditional): implement static placement annotations on DuoKV's retrieval-head slab + streaming-head ring driven by the theoretical optimum — concretely, partition the slabs across `mx.metal.set_residency_*` levels (whichever MLX exposes) according to the LP solution.
- **Verify**: (a) Phase 0 instrumentation produces `bench/snapshots/devloop-N/tier_residency.json` with per-tier hit/miss counts at 2K/4K/8K/16K/32K decode contexts. (b) Theoretical upper-bound curve produced as `research/analyst_runs/2026-04-26/kv_placement_upper_bound.md` with the LP formulation, per-tier bandwidth/capacity values, and the resulting `tok/s_max(T)` function. (c) Diagnostic memo: explicit go/no-go decision (proceed-to-Phase-1 vs dead-end) with the bound-vs-measured ratio at each context length. (d) Phase 1 (if triggered): unit tests in `tests/test_kv_placement.py` covering LP solver correctness on the canonical 4-tier example from the paper. (e) Phase 1: `efficiency_profile --decode-ctx 32768 --kv-mode duo --decode-n 64 --kv-placement bounded` produces decode tok/s ≥10% above baseline at 32K (target: recover toward 11-12 tok/s) AND no Metal peak regression vs baseline. (f) Phase 1: `hypercar_bench --full --kv-placement bounded` passes all 11 gates with no quality regression — placement is residency-only. (g) Phase 1: a tiny perturbation gate — flip `--kv-placement bounded` off and confirm performance returns to baseline (proves the gain is from placement, not from a confounded code path).
- **Effort**: S-M (Phase 0 + analysis = 2-3 days; Phase 1 implementation = 1-2 weeks if Phase 0 passes the >2× gate, otherwise dead-end-and-document = 1 day)
- **Depends on**: Task 303 (DSA empirical cache-locality diagnostic) for ground-truth measurements to validate the bound against. Task 290 (32K efficiency_profile baseline) shipped — provides the bound's reference number. No model dependency; the placement math is model-agnostic. **Microbench Phase 0 gate is required before any Phase 1 implementation work** per the 2026-04-26 feedback memory — the paper's Grace Hopper / discrete-tier framing is a known mismatch with Apple's UMA, so the upper-bound calculation MUST be sanity-checked against measured tok/s before committing to integration.

### 314. DASH-KV asymmetric hashing as a third KV mode — O(N²)→O(N) attention complement to TQ3/DuoKV/SnapKV
- **Goal**: 1 (1M context — the only attention-computation lever in the queue that scales to 1M without giving up exactness on retrieved tokens), Goal 3 (decode at long context, conditional on Phase 0 latency budget)
- **Derived from**: 2604.19351 "DASH-KV: Accelerating Long-Context LLM Inference via Asymmetric KV Cache Hashing" (Jinyu Guo, Zhihan Zhang, Yutong Li et al., ACL 2026 Findings, April 2026). Reformulates attention as approximate nearest-neighbor search via asymmetric deep hashing — queries and keys go through *different* hash networks (queries higher precision, recomputed every step; keys lower precision, cached). Adaptive mixed-precision retention keeps high-attention tokens at fp16. Complexity O(N²) → O(N). DASH-KV was deferred in pass 59 under the implementation-confidence filter for being arxiv-only at the time; ACL 2026 Findings acceptance lifts the provenance veto.
- **Change**: Phase 0 (microbench validation, 1-2 days). (1) Clone github.com/Zhihan-Zh/DASH-KV. Identify the hash-network architecture (binary projection LSH-style is most likely — assess from code). (2) Port one layer's hash network to MLX in `omlx/bench/dash_kv_probe.py` — only the query/key encoding + bucket-lookup path, no full integration. (3) Microbench hash-and-retrieve latency on M4 Pro at 16K and 64K decode contexts. (4) Microbench retrieval quality: measure Hit@K (where K = DuoKV retrieval budget) on a 1K-prompt sweep against full-attention top-K ground truth. **Acceptance gates**: hash-retrieve latency must be under 0.5 ms/layer/step at 16K (within DuoKV's per-step budget) AND Hit@K ≥ 95% at K = retrieval budget. If both pass, proceed to Phase 1. If either fails, file as research-deferred (algorithmic merit confirmed, Apple-Silicon-port cost dominates) and document in `research/analyst_runs/2026-04-26/dash_kv_validation.md`. Phase 1 (3-4 weeks, conditional on Phase 0): integrate as a third KV-mode `--kv-mode dash` in `omlx/dash_kv_cache.py` alongside `duo`/`tq3`/`native`. Implement the full asymmetric encoding + adaptive mixed-precision retention. Phase 2 (optional): combine with DuoKV split — DASH on retrieval heads, DuoKV streaming ring on streaming heads (asymmetric encoding maps cleanly onto streaming-vs-retrieval split, since streaming heads need exact recent tokens and retrieval heads can tolerate hashed approximation).
- **Verify**: (a) Phase 0 microbench produces `bench/snapshots/devloop-N/dash_kv_phase0.json` with hash-retrieve latency at 16K and 64K plus Hit@K quality numbers vs full-attention ground truth. (b) Validation memo `research/analyst_runs/2026-04-26/dash_kv_validation.md` documents go/no-go decision and any Apple-Silicon-port frictions encountered. (c) Phase 1 (if triggered): unit tests in `tests/test_dash_kv.py` covering asymmetric encoding correctness, bucket-lookup determinism, and adaptive mixed-precision retention thresholds. (d) Phase 1: `efficiency_profile --decode-ctx 65536 --kv-mode dash --decode-n 64` produces decode tok/s ≥ 8 tok/s at 64K (target: matches or exceeds DuoKV+SnapKV at 64K, validating the O(N) complexity advantage at the regime where DuoKV's O(N²) gather would dominate). (e) Phase 1: NIAH at 64K with `--kv-mode dash` PASSES (≥95% retrieval — the asymmetric encoding's whole pitch is that it preserves long-context retrieval). (f) Phase 1: `hypercar_bench --full --kv-mode dash` passes all 11 quality gates — adaptive mixed-precision retention is the safety mechanism for high-attention tokens. (g) Phase 1: Metal peak at 64K decode with `--kv-mode dash` does NOT exceed the Goal-6 ceiling (currently 40.7 GB; allowance: ≤41.0 GB).
- **Effort**: M-L (Phase 0 + validation memo = 1-2 days; Phase 1 integration = 3-4 weeks if Phase 0 passes both gates, otherwise research-deferred = 1 day)
- **Depends on**: No hard dependency on shipped Hypercar work — DASH-KV is a parallel KV-mode. Soft dependency on Task 313 (theoretical placement bound) — if Task 313 finds we're already near the placement ceiling (i.e., the cliff is purely algorithmic), DASH-KV's O(N²)→O(N) advantage becomes the highest-priority response. **Microbench Phase 0 gate is required before any Phase 1 integration work** per the 2026-04-26 feedback memory — the paper has no Apple-Silicon validation, and the per-layer hash-network compute cost is the failure mode we'd burn 3-4 weeks integrating before discovering. The 0.5 ms/layer/step latency target maps to 24 ms/decode-step total at 48 layers — DuoKV's current 16K decode-step budget is ~62 ms (16 tok/s), so DASH must fit comfortably under half that.

## Analyst run 2026-04-26 (measurement-backed tickets, branch `analyst/2026-04-26`)

Source: `research/analyst_runs/2026-04-26/findings.md`. Each ticket cites the
finding (M1-M6) it derives from and the artifact under `research/analyst_runs/2026-04-26/`
that backs it. Tickets are numbered 305-312 (304 was the latest pre-cron ticket).
P0 = decision-blocker for queued architectural work. P1 = ship-soon, high
confidence. P2 = ship-eventually or follow-up audit.

### 305. DuoKV trim arange-identity slice fast-path — replace 219x-587x slower gather with slice
- **Goal**: 3 (decode tok/s at long context — gather appears 48x per decode step in DuoKV's trim path), Goal 6 (slice produces a no-copy view; gather materialises a buffer the size of the gathered output, so this is also memory-positive at 16K+)
- **Derived from**: M1 in `research/analyst_runs/2026-04-26/findings.md`. Synthetic microbench `tools/analyst_kit/claims_2026_04_26.py` (test "F1b") shows `mx.broadcast_to(idx, shape) + mx.take_along_axis(x, idx, axis=2)` is **219.18x slower** than `x[:, :, :T, :]` at T=16384 and **587.50x slower** at T=65536 on `(1, 4, T, 128)` fp16, with bit-identical output. Evidence file: `research/analyst_runs/2026-04-26/claims.json` -> `f1b_gather_vs_slice` block.
- **Change**: Audit DuoKV's trim path (`omlx/duo_kv_cache.py` -- on the in-flight `hypercar` branch this is the 605-line version with `_TRIM_INDEX_CACHE`; on this analyst branch it's the 467-line e0ed34c version) for sites that build a per-head index tensor via `mx.broadcast_to(arange, ...) + take_along_axis`. For each site, add a runtime fast-path: if every retrieval head's index is `arange(T)` (i.e. no eviction has happened on that head yet), replace the gather with a slice. Concretely: (1) at trim-policy-build time, set a per-layer `_is_identity_trim` flag when all retrieval heads are full and no streaming evictions have fired yet. (2) At trim-execution time, branch on the flag: identity -> slice; otherwise -> existing gather. (3) Make the flag invalidate as soon as any head crosses the trim threshold (stays true through prefill + early decode, then flips to gather once the streaming heads start evicting). The check is one `mx.array_equal` at flag-build time, not on every call.
- **Verify**: (a) Unit test in `tests/test_duokv_split_phase_a.py` (or new `tests/test_duokv_identity_slice.py`) constructs a DuoKVCache primed to capacity-1 then to capacity+1 and asserts: identity-flag is True before eviction, False after, slice path is taken when True (assert via tracer counter), gather path when False. (b) `tools/analyst_kit/claims_2026_04_26.py` run on the patched branch shows the slice path active during early decode (counter `duokv.trim.slice_fastpath_calls > 0`). (c) `efficiency_profile --decode-ctx 16384 --kv-mode duo --decode-n 64` decode tok/s improves by >=5% over the same branch's pre-patch baseline at 16K, where the optimization is most measurable (early-decode steps dominate before eviction kicks in for short generations). (d) `hypercar_bench --full` passes all 11 gates with no quality regression -- the optimization is bit-identical so this is a sanity gate.
- **Effort**: S (3-4 days -- 1 day audit + flag design, 1 day implementation, 1 day test, 1 day bench validation)
- **Priority**: **P1** -- highest-confidence measured speedup in the audit.
- **Depends on**: None. Compatible with Tasks 269/271 (module-level trim-index cache); the identity-flag fast-path short-circuits *before* the cached-index path runs, so they compose.

### 306. StreamingKV ring arange-identity slice fast-path — same pattern in the streaming buffer
- **Goal**: 3, Goal 6 (same rationale as Task 305)
- **Derived from**: M1 in `research/analyst_runs/2026-04-26/findings.md`. Same F1b microbench result. NF6 in `00_session_log.md` notes that StreamingKVCache.update_and_fetch is fast on its own (0.042ms median, claims.json `streamkv_ring` block), but anywhere it builds a gather-shaped index for the ring view, the F1b ratio applies. Evidence file: `research/analyst_runs/2026-04-26/claims.json`.
- **Change**: Audit `omlx/streaming_attention.py` (and `_gather_per_head_view` if present) for sites where a ring-buffer view is constructed via gather when all heads have the same canonical positions. Add the same identity-flag fast-path as Task 305: when no head has rotated past wrap, the view is contiguous and slice-able; otherwise gather. Cost is one comparison per layer per decode step.
- **Verify**: (a) Unit test in `tests/test_streaming_attention.py` (or equivalent) constructs a StreamingKVCache pre-wrap and post-wrap, asserts slice path active in the first regime, gather in the second. (b) Tracer counter `streamkv.ring_view.slice_fastpath_calls > 0` in pre-wrap traced runs. (c) `efficiency_profile --decode-ctx 8192 --kv-mode duo --decode-n 64` decode tok/s improves by >=2% on the patched branch (smaller absolute gain than Task 305 because the ring view is smaller, but same direction). (d) `hypercar_bench --full` passes all 11 gates with no quality regression.
- **Effort**: S (2-3 days -- pattern is the same as Task 305, half the audit surface).
- **Priority**: P2 -- same evidence as Task 305 but smaller absolute impact (ring is smaller than retrieval cache); ship after 305 lands.
- **Depends on**: Task 305 design (use the same identity-flag pattern; do not invent two different flag schemes for the same underlying optimisation).

### 307. mx.zeros_like → scalar 0.0 sweep across DuoKV / SnapKV / streaming `where` operands
- **Goal**: 3 (per-decode-step reduction), Goal 6 (eliminates an unnecessary T-shaped allocation per call site at long context -- the zero-tensor that `zeros_like` materialises before `where` consumes it).
- **Derived from**: M2 in `research/analyst_runs/2026-04-26/findings.md`. `claims_2026_04_26.py` test "F1a" shows `mx.where(mask, x, mx.zeros_like(x))` is **1.40x slower at T=16384** and **2.29x slower at T=65536** than `mx.where(mask, x, 0.0)`, bit-identical output, no heap-delta change across repeats. 48-layer absolute saving at 16K = 0.42ms/decode step (~0.4% of decode wall at 9.92 tok/s). Evidence file: `research/analyst_runs/2026-04-26/claims.json` -> `f1a_zeros_like_vs_scalar` and `f1_48layer_simulation` blocks.
- **Change**: `grep -nE "mx\\.where\\(.*mx\\.zeros_like" omlx/**/*.py` and replace each `mx.zeros_like(x)` argument to `mx.where` with the scalar `0.0` (or appropriate dtype-matched scalar -- `mx.where` casts scalars at dispatch). Do NOT replace `mx.zeros_like` outside of `where` operands; in other contexts it may be load-bearing for shape inference. Audit surface: DuoKV trim mask path, SnapKV freshness-score band mask, streaming attention ring mask. Likely 3-6 call sites total based on yesterday's static-review pass.
- **Verify**: (a) Unit test asserting bit-identical output of patched and pre-patch versions on a fixed seed input (regression gate; the change is bit-identical by design). (b) `claims_2026_04_26.py` re-run on the patched branch confirms F1a's 1.40x ratio realises end-to-end. (c) `efficiency_profile --decode-ctx 16384 --kv-mode duo --decode-n 64` decode tok/s improves by >=0.4% (within noise but should be in the right direction; the value is in cumulative effect across many call sites, not per-site).  (d) `hypercar_bench --full` passes all 11 gates with no quality regression.
- **Effort**: S (1-2 days -- mostly grep + edit + unit test; the optimization is one keystroke per call site).
- **Priority**: P2 -- low-risk, low-individual-impact, but worth doing as a cleanup pass alongside Task 305/306.
- **Depends on**: None.

### 308. Re-derive decode-cliff hypothesis with per-op timer breakdown — block on this before further architectural work
- **Goal**: 3 (the cliff at 16K->32K decode is the most concrete remaining Goal-3 gap; getting the diagnosis right is the gate to picking the correct architectural fix).
- **Derived from**: M3 + M4 in `research/analyst_runs/2026-04-26/findings.md`. The CLAUDE.md framing ("DuoKV gather O(T_total) per call") is **not supported** by the single-call scaling curve we measured: 1024->4096->8192->16384 medians were 0.217 / 0.256 / 0.415 / 0.272ms with 16K *lower than* 8K. End-to-end `update_and_fetch` was ~21% of decode wall at 16K -- a minority cost. Implementer's own analysis in `omlx/patches/duo_split_attention.py:19-29` ("decode is dominated by MoE/projections, not KV gather") is consistent with the measurement and *contradicts* the CLAUDE.md framing. Several queued architectural tickets (Task 265 Fix 2, Task 303 DSA cache-locality) are scoped to the gather-dominance hypothesis; if that hypothesis is wrong, those tickets are mis-scoped. Evidence files: `research/analyst_runs/2026-04-26/claims.json` -> `update_and_fetch_curve`, `traced_decode_4K.json`, `traced_decode_16K.json`.
- **Change**: Phase 0 (1-2 days, this is the whole ticket): Run `traced_decode.py` at 4K / 8K / 16K / 32K with the tracer extended to wrap **all** Qwen3 forward-pass components, not just DuoKVCache. Specifically: (1) wrap `qwen3_moe.py` MoE block (gate, argpartition, gather_qmm) via `trace_module` already in the scaffold, (2) wrap attention projection (Q/K/V projections and output projection) via the same mechanism, (3) wrap `lm_head` via decorator on the model class after load. Run each context length, dump `registry.snapshot()` as JSON, write `research/analyst_runs/2026-04-2N/decode_op_breakdown.md` with a stacked-bar plot of per-op share at each context length. Phase 1 (gated on Phase 0): if the breakdown shows MoE/projections dominating (as implementer's analysis predicts), open follow-up tickets for MoE expert-dispatch optimization (Task 312 is the seed). If the breakdown shows the gather dominating after all (the CLAUDE.md framing), keep Tasks 265/303 as scoped. **Do not start Tasks 265 Fix 2 or 303 implementation until this measurement is in.**
- **Verify**: (a) `research/analyst_runs/2026-04-2N/decode_op_breakdown.md` exists with per-op share at 4K/8K/16K/32K. (b) The top-5 ops at 16K and 32K are explicitly named with timer.total_s. (c) The decision in Phase 1 is documented (continue Tasks 265/303 as-is OR open follow-up tickets) with the measurement evidence cited inline. (d) If the analysis takes >2 days, raise to user; this ticket is a measurement audit, not an implementation.
- **Effort**: S (1-2 days -- measurement only, no production code changes).
- **Priority**: **P0** -- blocker on Tasks 265 Fix 2, 303 (architectural decisions whose scope depends on this answer).
- **Depends on**: None. Re-uses the existing observability scaffold + traced_decode template; the only new code is `trace_module` calls on qwen3_moe and attention projection.

### 309. Tracer-overhead calibration baseline — quantify the gap between traced and un-traced decode
- **Goal**: 2 (intelligence claims are credible only if measurement is credible; tracer overhead currently confounds traced-decode tok/s readings -- M4 showed traced 4K=20.6 tok/s vs CLAUDE.md 4K=44 tok/s on the in-flight branch, a ~2x gap that mixes branch differences with tracer overhead).
- **Derived from**: M4 in `research/analyst_runs/2026-04-26/findings.md`. The traced 4K decode tok/s (20.6) is roughly half the published un-traced number (44 tok/s on the in-flight `hypercar` branch). Two explanations are possible: (a) Tasks 269/271 contribute much more than +16% (they aren't on this branch), (b) tracer overhead is non-trivial. Without calibration, future analyst runs cannot attribute speed deltas correctly. Evidence files: `research/analyst_runs/2026-04-26/traced_decode_4K.json`, `traced_decode_16K.json`.
- **Change**: Phase 0 (1 day): write `tools/analyst_kit/tracer_overhead_calibrate.py` that runs the same model + cache at the same context length **twice**: once with `trace_class(DuoKVCache, mlx=True)` installed and once without (use `OMLX_OBSERVABILITY=0` env to disable, or simply skip the `trace_class` calls). Compare decode tok/s. Repeat at 4K, 8K, 16K to see if overhead is constant or scales with call-count. Output: `research/analyst_runs/2026-04-2N/tracer_overhead.json` with `{ctx, traced_tok_s, untraced_tok_s, overhead_pct}` rows. Phase 1 (gated on Phase 0): if overhead is >=10%, document the correction factor and update `tools/analyst_kit/traced_decode.py` docstring with a "subtract this from absolute decode tok/s" warning. If overhead is <5%, the explanation collapses to (a) above and the +16% claim for Tasks 269/271 is conservative -- file a follow-up ticket to update CLAUDE.md.
- **Verify**: (a) `research/analyst_runs/2026-04-2N/tracer_overhead.json` exists with overhead_pct measured at 3+ context lengths. (b) `tools/analyst_kit/traced_decode.py` docstring updated with the calibration result if overhead >=10%. (c) If overhead <5%, `CLAUDE.md` line 168 ("DuoKV trim mx.array cache (Task 271): +2.9% incremental at 16K") is updated with a follow-up note that Tasks 269+271 combined may contribute much more than the cited deltas, citing this calibration. (d) The harness is repeatable (re-running produces consistent overhead within +/-2%).
- **Effort**: XS (0.5-1 day -- two runs of an existing script with a flag flip).
- **Priority**: P1 -- gates the credibility of every traced-decode finding from this audit forward.
- **Depends on**: None. Uses scaffold that already exists.

### 310. 64K traced-decode + Task 149 pre-alloc 248x claim verification
- **Goal**: 1 (validates the 64K decode tok/s on the path to 1M context), Goal 2 (closes a stale-claim gap -- Task 149's 248x claim is in CLAUDE.md but un-verified by independent measurement).
- **Derived from**: "What this run did NOT establish" in `findings.md`. The `update_and_fetch` scaling curve in M3 stops at 16K because the synthetic test budget didn't extend further. Task 149 (DuoKV pre-allocated slab + O(1) slice write) claims **248x decode at 64K**; CLAUDE.md line 152 cites it but no claims-harness entry exists for it. This is exactly the kind of stale-claim re-verification the cron prompt mandates.
- **Change**: Phase 0 (0.5 day): extend `tools/analyst_kit/claims_2026_04_26.py` (or write `claims_2026_04_27.py`) with a `Task149_PreAllocSlab` claim that times decode-shape append on a 64K-primed cache, baseline = synthesize a "would-have-been" naive-realloc cache (manually concat per-step) vs optimized = current pre-alloc slab. Phase 1 (1 day): write `tools/analyst_kit/traced_decode_64K.py` (or extend the existing tool with `--context-tokens 65536` -- it's already parametric), run end-to-end at 64K, dump JSON. Memory budget: 64K decode peak from CLAUDE.md is ~38 GB; well within ceiling. Time budget: ~6 minutes prefill + 64-step decode at ~5 tok/s = ~13 seconds. Total ~7 minutes per run.
- **Verify**: (a) `claims.json` includes a Task149_PreAllocSlab entry with verdict PASS/SUSPECT/FAIL. (b) If FAIL, file a follow-up ticket noting the negative result -- "Task 149 248x claim does not reproduce on M4 Pro 48GB at 64K, measured ratio is X". (c) `research/analyst_runs/2026-04-2N/traced_decode_64K.json` exists with prefill tok/s, decode tok/s, Metal peak, phys_footprint at 64K. (d) Memory caps respected (no breach during the run). (e) Decode tok/s reported within 2x of CLAUDE.md's 5 tok/s extrapolation -- if wildly off, document as bench-anomaly and re-run.
- **Effort**: S (1-2 days -- extend existing tools, run, write up).
- **Priority**: P2 -- not blocking any architectural decision; worth doing as the natural follow-up to this audit.
- **Depends on**: Task 309 (tracer-overhead calibration) ideally, so the 64K decode tok/s can be reported corrected. Soft dependency.

### 311. TQ3 fused-quantize 15x claim verification — second stale-claim re-test from CLAUDE.md
- **Goal**: 2 (CLAUDE.md cites a 15x short-prefill speedup from Task 152's WHT fused dense kernel; this is one of the largest single claims in the doc and has no claims-harness entry).
- **Derived from**: "What this run did NOT establish" in `findings.md`. CLAUDE.md line 154 ("TQ3 fused quantize (Task 152): 15x short-prefill, 1.33x at 8K") -- the same kind of un-verified microbench-level claim that motivated this entire audit. The cron prompt mandates re-verifying >=4 stale claims; this audit re-verified F1a, F1b, the update_and_fetch curve, and the StreamingKV ring overhead -- meeting the bar. Task 311 extends the bar.
- **Change**: Phase 0 (0.5 day): write `tools/analyst_kit/claims_tq3_2026_04_2N.py` with a `Task152_FusedQuantize` claim. Baseline = naive per-row WHT quantize via the un-fused path (locate it via `git log -p -- omlx/turboquant_kv.py` to find the pre-Task-152 code). Optimized = current fused dense kernel. Time on synthetic `(1, 4, T, 128)` fp16 inputs at T=512 (short prefill, where 15x claim lives), T=2048 (mid), T=8192 (where 1.33x claim lives). Use the existing `Claim`/`run_claim` infrastructure from `omlx.observability.claims`. Phase 1 (gated on Phase 0): if either claim FAILs, file a follow-up ticket. If both PASS, update CLAUDE.md with a citation to the claim entry (so future audits don't re-do this work). If SUSPECT (within tolerance but not exact), no action -- numbers drift across hardware/MLX versions and that's expected.
- **Verify**: (a) `claims_tq3.json` includes Task152_FusedQuantize entries at T=512, 2048, 8192 with verdicts. (b) Bit-identical output check passes (the fused kernel must produce the same output as the naive path within fp16 precision; tolerance 1e-3). (c) If 15x at T=512 reproduces, CLAUDE.md gains a footnote `^claims/2026-04-2N/Task152_FusedQuantize` citing the claim entry. (d) If 15x does not reproduce, `findings.md` for the run (or a `tq3_followup.md`) documents the gap.
- **Effort**: S (1 day -- straightforward microbench using the existing claims harness).
- **Priority**: P2 -- not blocking; valuable for closing the stale-claim audit loop on TQ3 mode.
- **Depends on**: None. Uses scaffold that already exists.

### 312. MoE expert-dispatch decode-share probe — measure the implementer's hypothesis
- **Goal**: 3 (if MoE expert-dispatch is the dominant decode cost as the implementer's analysis suggests, then per-token expert prefetching, expert clustering, or DyMoE-style runtime orchestration becomes the right Goal-3 fix -- not the gather optimization that current architectural tickets are scoped to).
- **Derived from**: NF6 in `research/analyst_runs/2026-04-26/00_session_log.md` + M3, M4 in `findings.md`. NF6 documents that `mlx_lm/models/qwen3_moe.py:110-139` dispatches 8 experts via a SINGLE `gather_qmm` call (not 8 separate dispatches), so the dispatch *count* is small (~3 per layer) but the dispatched *work* (8 experts x MLP per token) is substantial. M3/M4 showed `update_and_fetch` is only ~21% of decode wall at 16K, leaving ~80% in the rest of the forward pass; `omlx/patches/duo_split_attention.py:19-29` claims MoE/projections dominate that ~80%. This ticket measures it directly. Sister-ticket to Task 308 (which also measures the breakdown but for the architectural-decision angle); 312 is scoped specifically to MoE.
- **Change**: Phase 0 (1 day): write `tools/analyst_kit/moe_decode_probe.py` that uses `trace_module(qwen3_moe, mlx=True)` from the observability scaffold to time `Qwen3MoeSparseMoeBlock.__call__`, the gate softmax, the argpartition, and the SwitchGLU gather_qmm separately. Run end-to-end at 4K/8K/16K/32K (32K only if memory allows; defer to Task 308's 32K run otherwise). Output: `research/analyst_runs/2026-04-2N/moe_decode_probe.json` with per-component timer breakdowns. Phase 1 (gated on Phase 0): if MoE block is >=40% of decode wall at 16K+, file an architectural ticket for MoE optimization (expert prefetching, expert pinning to high-bandwidth VM regions, or DyMoE-style routing -- pick the one matching the breakdown shape). If <20%, the implementer's hypothesis is wrong and the Goal-3 fix is elsewhere; document in findings.md.
- **Verify**: (a) `moe_decode_probe.json` exists with per-component breakdowns at >=3 context lengths. (b) The dominant component (>=50% of MoE block time) is named explicitly with its timer.total_s. (c) The follow-up architectural ticket is filed (or a "no-op, hypothesis refuted" memo is written) within Phase 1's budget. (d) The probe does not change `omlx/patches/qwen3_moe.py` or any production file -- it is measurement-only and lives in `tools/analyst_kit/`.
- **Effort**: S (1-2 days -- straightforward use of the scaffold).
- **Priority**: P1 -- decision-grade input for Goal 3 architectural choices; pairs with Task 308 to give a complete decode-time accounting.
- **Depends on**: Task 309 (tracer-overhead calibration) ideally so MoE-share numbers are corrected. Soft dependency. No other dependencies.
