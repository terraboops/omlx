...
- **Task 263**: Fix `efficiency_profile.py` KeyError + add decode-at-context profiler — **shipped + surprising Goal 3 finding** (2026-04-24)
  - **Fixes shipped**:
    ...
    - `efficiency_profile.py` CLI: added proper `argparse` with `--decode-ctx N` and `--decode-n N` flags. Previously `--help` accidentally triggered the full profile run because there was no argparse at all.
  - **New tool**: `profile_decode_at_context(context_tokens, n_decode)` — loads the real model, prefills `context_tokens` of filler text, times `n_decode` steady-state greedy decode tokens (first decode step excluded to avoid one-shot compile cost). Emits prefill tok/s, decode tok/s, per-step ms, Metal peak/active. Opt-in via `--decode-ctx N` (default: run all original micro-benchmarks).
  - **Surprising Goal 3 finding** from running it at 16K:
    - **Decode @ 16K = 27.15 tok/s** (36.8 ms/step, 32 steady-state steps)
    - Prefill @ 16K = 405 tok/s (39.5s for 16K tokens)
    - Metal peak = 41.07 GB (at the 41.2 GB ceiling — tight)
    - **This is ~2× faster than the NIAH phase reports (13.3 tok/s in hypercar_bench Phase 3)**. The NIAH measurement includes something (maybe warmup contamination, maybe specific prompt-shape effects, maybe the extra 128-token decode-stress loop running over edge-case tokens) that drags the rate down. Under clean steady-state on a non-eval prompt, actual decode @ 16K is 27 tok/s.
    - **Goal 3 real gap**: 50 − 27 = 23 tok/s, not the 50 − 13 = 37 tok/s the CLAUDE.md status row implied. Still below target but much more achievable.
  - **Implication for Goal 3 planning**:
    - Back-of-envelope bandwidth model (~19 GB reads/token at 273 GB/s = ~14 tok/s) was pessimistic. Real measurement is 27 — so something is better than worst case. Maybe DuoKV's 59% streaming heads are kicking in and reducing effective reads.
    - To reach 50 tok/s: need ~1.9× speedup. Options: Quest page selection (only attend to top-K pages = 20-30% of KV reads), sparse attention (MInference, HiP), more aggressive DuoKV (more heads classified as streaming).
    - Phase 3's "decode_toks" metric as reported by the bench is NOT a reliable Goal 3 measurement. The NIAH phase mixes decode with short-answer/eos handling. For a true Goal 3 regression metric we should use the `--decode-ctx 16384` profiler output.
  - **Follow-up**:
    - (i) **Update CLAUDE.md Goal 3 row** with the 27 tok/s steady-state number (cycle-sized).
    - (ii) Investigate the NIAH vs steady-state discrepancy (why is NIAH decode 2× slower?) — tiny bench modification to log both.
    - (iii) With a reliable measurement tool, start evaluating Quest / sparse-attention tasks that are on the backlog (Task 3 Quest, Task 5 MInference, Task 183 InfiniteHiP).
  - **No bench run needed to validate this task** — the new function has a test-via-usage pattern (ran it once, got expected output shape, exit 0, numbers in range). Tool is ready for future Goal 3 investigations.
- **Task 262**: LCB few-shot probe (DP stairs example) — **REGRESSION, reverted** (2026-04-24)
  - **Hypothesis**: add one worked DP example (climb-stairs problem) to the LCB prompt to prime the model on the problem→pattern→code reasoning template. Might unlock medium problems the model can solve but fails to identify the right approach for.
  - **Bench result**: LCB **7/20 (35%) — DOWN from 8/20 baseline**. Zero gains, one regression (Fill the Gaps, which the retry logic had been recovering).
  - **Hypothesis FALSIFIED + UNDESIRABLE**: the DP example biased the model toward DP thinking on non-DP problems. Fill the Gaps is an iterative-insertion problem — the DP few-shot seems to have steered retries at temp=0.7 away from the iterative-rebuild solution that was working into a DP-shaped wrong algorithm.
  - **Medium/hard unchanged**: same 0 PASSes among retries. The few-shot didn't help the capability ceiling as hoped. Makes sense in hindsight — medium/hard problems on this bench span DP, greedy, binary search, modular arithmetic, string manipulation, graph. A SINGLE DP example can only bias toward DP, which is a net negative when the current problem isn't DP.
  - **Decision**: reverted. Comment in `hypercar_bench.py` documents the attempt so future cycles don't re-try a single-example few-shot.
  - **What might still work** (deferred follow-ups):
    - (i) **Problem-class-matched few-shot**: classify each problem by pattern at run-time (regex or a small classifier on the description), then show a matching worked example. Much bigger effort and adds a new failure mode (wrong classification → wrong example → same regression).
    - (ii) **Multi-example few-shot** (DP + greedy + binary-search): crowds the prompt toward 1-2K tokens of examples, risks eating the 2048-token output budget. Worth a probe but substantial design work.
    - (iii) **Multi-sample best-of-N every problem** (not just on sample-fail): more expensive (2-3× bench time) but potentially useful — the Task 260/261 data shows greedy passes sample for 7 problems, but some of those might fail hidden tests and a different sample might pass.
  - **MMLU-Pro stable**: 62/100, Metal flat at 35.1 GB across all 100 Qs. Task 259 fix continues to hold (4 consecutive runs now). Bench reliability is solid.
  - **Takeaway**: static few-shot on LCB's diverse problem set is FRAGILE. Any further LCB work on this model should focus on either (a) dynamic/matched few-shot, or (b) accept 8/20 and move to Qwen3.6 or Goal 3/4 speed work.
- **Task 261**: Higher-variance retry (temp=0.7→1.0, N=2→3) — **NEUTRAL, reverted** (2026-04-24)
  - **Hypothesis**: Task 260's retry logic won 1/13 candidates at temp=0.7, N=2. Higher variance (temp=1.0, more retries) might unlock 1-3 more on medium/hard problems.
  - **Bench result**: LCB **8/20 (40%) — identical to Task 260**. Same single winner (Fill the Gaps, retry 1). Per-difficulty unchanged: easy 7/8, medium 1/8, hard 0/4.
  - **Cost**: 37 retry generations vs 25 at 0.7/N=2 (+48% generations for zero PASS gain). Phase 3d 2093s vs 2029s (+3%).
  - **Hypothesis FALSIFIED**: medium/hard LCB fails are NOT sampling-bounded on this model — 11/11 medium+hard retry candidates still failed the sample test at temp=1.0. The same one that was "rescuable" via retry at temp=0.7 (Fill the Gaps) is still rescuable; nothing new. The LCB ceiling for Qwen3-Coder-30B-A3B-8bit on this problem set is **8/20 with retry, 7/20 without**.
  - **Decision**: reverted to LCB_RETRIES=2, LCB_RETRY_TEMP=0.7. Comment updated to document the 1.0-vs-0.7 comparison so future cycles don't re-try.
  - **MMLU-Pro still stable** this run: Metal peak flat at 35.1 GB throughout all 100 questions. Task 259 fix continues to hold.
  - **Follow-up threads**:
    - (i) **Only remaining LCB lever on this model**: few-shot examples (Option B) — add worked examples of problem classes the model consistently fails (DP, greedy-optimization, string manipulation, modular arithmetic). Expected value lower than Qwen3.6 migration but not blocked.
    - (ii) **Qwen3.6 migration (Task 253)** is the next big lever. Current target Qwen3-Coder-30B-A3B-8bit has a provable 40% LCB ceiling with all sampling/retry strategies exhausted. Different model = different ceiling.
    - (iii) **Alternate lever: pivot to Goal 3/4 speed work**. LCB is now at 40% with retry logic shipped; may be time to diversify.
- **Task 260**: Validation bench — BOTH Task 258 retry AND Task 259 MMLU-Pro fix validated (2026-04-24)
  - **Bench**: `--full`, started 08:03, finished 09:00 (57 min), ALL GATES PASSED.
  - **LCB: 8/20 (40%) — 2nd improvement. Session cumulative: 30% → 40%, +2 PASSes, +10 percentage points from Run 87 baseline.**
  - **Task 259 fix validated**: Metal peak flat at 35.1 GB / active 32.4 GB across all 100 MMLU-Pro questions (Q20, Q40, Q60, Q80, Q100 all identical). No slowdown at Q65+, no thrashing. MMLU-Pro completed in 1018s (17 min) — back to baseline. The `gc.collect() + mx.clear_cache()` per-question insert works.
  - **Task 258 retry validated**: retry logic fires when greedy fails `inputs[0]`; won 1/13 attempts (Fill the Gaps recovered on retry-1 at temp=0.7, seed=1). 25 total retry generations (12 problems × 2 retries + 1 winner on retry-1). Cost: Phase 3d 652s → 2029s (3× slower).
  - **Per-difficulty breakdown**:
    - easy 7/8 (87.5%) — UP from 6/8 baseline (Fill the Gaps recovered)
    - medium 1/8 (12.5%) — unchanged; 6/7 fails tried retries, none passed sample
    - hard 0/4 (0%) — unchanged; all 4 tried retries, none passed sample
  - **Interpretation of retry ceiling**: medium/hard fails are capability-limited, not sampling-limited. temp=0.7 retries don't produce algorithmically-different enough solutions. Higher temp or more retries would ~linearly inflate cost for marginal gains. Only easy-problem regressions (where CoT led to buggy-but-not-fundamentally-wrong code) benefit from retry.
  - **Follow-up threads**:
    - (i) **Higher-variance sampling** (temp=1.0, nucleus top-p=0.9) may unlock a few medium problems. Low-risk experiment — just change constants. Cycle-sized.
    - (ii) **Few-shot examples** for the 4 classes of failing problems (DP, greedy, modular-arithmetic, string-manipulation) — more expensive but targets the actual capability gap.
    - (iii) **Qwen3.6 migration (Task 253)** — biggest remaining single lever, expected to change the capability ceiling itself. Blocked on user-initiated 18 GB download.
- **Task 259**: Root-cause the Task 258 MMLU-Pro slowdown — **found and fixed** (2026-04-24)
  - **Root cause**: `phase3c_mmlu_pro` (line 1188 before fix) had no `gc.collect()` / `mx.clear_cache()` between questions. Every other bench phase (NIAH, RULER, LCB, HumanEval) does per-iteration cleanup; MMLU-Pro was the outlier. Over 100 questions, transient tensors (KV caches from `_generate()`, logits, intermediate activations) accumulated in Metal memory. At some threshold (Q≈65-70 on Task 258's run), the cumulative pressure tipped into swap, and each subsequent token decode started paging — 100× slowdown per question.
  - **Why it didn't manifest in Tasks 255/256 (which also lack the cleanup)**: those runs started with marginally more free memory and/or ran slightly faster such that accumulation stayed under the swap threshold for all 100 questions. The bug was latent and intermittent; Task 258's run happened to cross the line.
  - **Fix** (`omlx/bench/hypercar_bench.py:1223`): add `gc.collect(); mx.clear_cache()` at end of each MMLU-Pro iteration, matching the pattern every other phase already uses. Plus: log `mx.metal.get_peak_memory()` + `get_active_memory()` every 20 questions so future runs have memory-growth visibility in the console.
  - **Verification path (bench-gated)**: next --full run should (a) show stable Metal peak across MMLU-Pro (not growing linearly), (b) complete Phase 3c in ~15 min (baseline), (c) unblock Phase 3d to actually test Task 258's retry logic. If peak still grows, there's a deeper leak somewhere else in the stack.
  - **Tests**: 14 tests (LCB + model_constants) still PASS. This change is a simple cleanup insert and doesn't affect correctness.
  - **Note**: this finding also applies as a pattern — when adding a new phase with a per-question loop, include the cleanup. Could be worth a helper decorator or a test asserting every phase's inner loop has `mx.clear_cache()`.
- **Task 258**: Sample-verify + retry in LCB phase3d — **code shipped, bench incomplete (killed)** (2026-04-23 / killed 2026-04-24)
  - **Code status**: `omlx/bench/hypercar_bench.py::phase3d_livecodebench` refactored to extract `_generate_once(prompt_tokens, temperature, seed)` and `_passes_sample(code, prob)` helpers, then wrap the main generation in: greedy-first → if fails sample[0] → up to `LCB_RETRIES=2` retries with `temp=LCB_RETRY_TEMP=0.7` and seeds 1, 2 → pick first sample-passing attempt. Falls back to greedy if no retry passes. Bookkeeping saved per problem: `attempt_used`, `sample_passed`, `retries` list.
  - **Unit tests added** (`tests/test_eval.py::TestLiveCodeBench::test_execute_code_roundtrip_for_sample_check`): sanity-check the `_execute_code` contract the retry loop depends on. All 10 LCB tests PASS.
  - **Bench validation FAILED to complete** — kicked off 2026-04-23T21:09, killed 2026-04-24 (exit 143, SIGTERM) after running 7+ hours without reaching Phase 3d. Retry logic was never exercised by the bench.
  - **Why it didn't complete**: MMLU-Pro (Phase 3c) went into a severe slowdown mid-run. Last good throughput was ~10s/question at [65/100] (22:14 local). After that: [70/100] took 2h 13min, [75/100] took 2h 46min, [80/100] took 3h 29min for 5 questions each — roughly 100× slower. Phase 3d never started. Root cause is almost certainly memory pressure + swap thrashing (a prior probe showed 9 GB cumulative swap I/O during this run). The retry-code is in phase3d so it wasn't implicated.
  - **Analyst-window collision**: bench was still running at 04:11 local, well inside the 00:00-06:00 analyst exclusive window. Killed immediately on discovery. This is a process failure on the loop's part — a bench started at 21:00 can easily span the analyst window if it slows down. Future fix: add a wall-clock deadline to bench runs (e.g. abort if not done by 23:30 local) OR cron-check elapsed time on every fire and kill stale benches. Noted in cron prompt update below.
  - **Code not reverted** — the retry logic is logically sound and unit-tested; the failure was infrastructural (memory pressure during Phase 3c), not algorithmic. Next cron cycle with clean memory should re-run.
  - **Follow-up threads**:
    - (i) **Root-cause the MMLU-Pro slowdown**: what changed between 22:14 and 00:27 that made the model go 100× slower? Candidates: background process memory grab, system throttling, Metal resource leak, swap accumulation over long bench runs. Check for any long-running bench Metal memory growth — the 2+ hour slowdown suggests Metal or Python heap isn't freeing between MMLU-Pro questions. This is a separate bug worth its own task.
    - (ii) **Re-run Task 258 bench** once root-cause (i) is understood OR on a clean system. If retry logic works as designed: LCB should move from 7/20 to 9-13/20 range.
    - (iii) **Bench hard-stop watchdog**: add a wall-clock deadline flag (e.g. `--max-runtime 3600`) so runaway benches can't span the analyst window.
- **Task 257**: Investigate [easy] Fill the Gaps regression under CoT — docs-only analysis (2026-04-23)
  - **Finding**: the regression is a **reasoning-to-code translation bug**, not a prompt-shape problem. The model's reasoning block was CORRECT ("insert between them"), but the implementation used the anti-pattern `a.pop(pos+1); a.pop(pos); then insert between` which destroys both endpoints before inserting the in-between values. Confirmed empirically: running the model's code on Sample Input 1 produces empty output instead of `2 3 4 5 4 3 2 1 2`. The sequence gets progressively destroyed across iterations.
  - **Implication**: CoT on this particular problem led the model to a clever but buggy in-place mutation approach. A simpler "build a new list" approach would work and is what baseline (pre-CoT, 512-token) likely produced. The CoT may have steered toward more complex algorithms that are harder to code correctly — a double-edged sword.
  - **Options evaluated**:
    - (a) Difficulty-gated CoT (no CoT for easy): may help but would ALSO lose Default Price and Exponential Plant PASSes which came from CoT on easy. NET negative or zero.
    - (b) Prompt tweak ("prefer simple algorithms"): unlikely to reliably redirect the model.
    - (c) Few-shot examples showing clean patterns: M effort, might help but risks overfitting.
    - (d) **Self-verify-on-sample + retry**: the problem statement includes sample I/O; we could run the generated code on the sample and retry on failure. This would catch exactly this bug cheaply.
    - (e) Best-of-N with temp>0: different samples likely produce different algorithm choices; picking by sample-pass would naturally weed out the pop-then-insert anti-pattern.
  - **Recommendation**: (d) and (e) compose well and are the logical next steps. (d) is cheaper (no extra generation cost per sample) and gives an objective quality signal — implements a basic "test your code before submitting" loop that should be standard for any competitive-programming eval harness. This finding elevates option (d) above raw best-of-N as the next concrete task.
  - **Accepting the regression**: net improvement from Task 255 (+1 PASS gained, -1 PASS lost, gross +2 gained -1 lost = +1 net) is POSITIVE. Investigating further in isolation isn't worth a full bench cycle unless it's part of a bigger change like (d) or (e).
  - **No code change this task**. Next task pick: implement (d) — sample-verify + retry loop in phase3d.
- **Task 256**: Bump LCB token budget 2048 → 4096 — **NEUTRAL result, reverted** (2026-04-23)
  - **Hypothesis**: 4/20 responses at Task 255's 35% run were truncated at the 2048 cap — all medium/hard FAILs. Bumping to 4096 could recover 1-4 of them.
  - **Bench result**: LCB **7/20 (35%)** — IDENTICAL to Task 255. Same per-problem pass/fail pattern.
  - **Hypothesis FALSIFIED**: 3 of 4 formerly-truncated problems now finish cleanly at 4096 (smallest-substring 9723 chars ≈ 2430 tok; apply-operations 10253 ≈ 2563 tok; continuous-subarrays 16583 ≈ 4145 tok) **but still FAIL** — the model's algorithm is wrong, not truncated. More reasoning budget = more wrong reasoning, not more correct answers. Only 1 problem (Guess the Sum, 4087 tok) still truncates at 4096, and another (maximize-amount-two-days, 3649 tok) newly reasons past the cap.
  - **Cost**: Phase 3d 652s → 948s (+296s ≈ +5 min). Full bench 1866s → 2161s. Every future bench run would pay this tax for no quality improvement.
  - **Decision**: REVERTED to 2048. The `MAX_LCB_TOKENS` constant comment now documents the 2048/4096 comparison so future readers don't re-try this.
  - **Real next lever identified**: best-of-N sampling with temp > 0 (Ranking Rule C) or few-shot examples for specific failing classes (Rule D). Token budget is no longer the bottleneck past 2048 for this model.
  - **Also observed**: HumanEval 19/20 (95%), MMLU-Pro 62/100, RULER/NIAH 100% — all unchanged. No side-effect regressions from the bump.
- **Task 255**: LCB CoT bench validation — **FIRST BENCH-RESULT IMPROVEMENT SHIPPED** (2026-04-23)
  - **Result**: LiveCodeBench **6/20 (30%) → 7/20 (35%)**. Net +1 PASS, +5 percentage points. ALL GATES PASSED.
  - **Gains (2)**: [easy] Default Price, [easy] Exponential Plant — both FAILED in baseline Run 87 AND the first Task 130(b) attempt; CoT + sufficient token budget now solves them.
  - **Loss (1)**: [easy] Fill the Gaps — model reasoned itself into a wrong approach. Net still positive.
  - **Unchanged** (passes still pass, fails still fail): 17 other problems — hard (0/4), most medium (1/7), the "always easy" passes (total-distance-traveled, minimum-number-of-chairs, Cutoff, A..B..C).
  - **Debugging journey (the real story)**:
    1. First re-run after just Task 254 (CoT prompt unified, 512-token cap untouched): **LCB 0/20** — catastrophic regression. CoT reasoning ate the 512 tokens, extractor fell back to line-by-line and picked up `## Problem Understanding` as "code."
    2. Response inspection (enabled by Task 254 instrumentation!) confirmed: response was 1938 chars, unclosed ```python block, model truncated mid-code.
    3. Two fixes applied: (a) bumped `range(512)` → 2048 in `hypercar_bench.py::phase3d_livecodebench`; (b) hardened `_extract_code` and `_extract_last_code_block` to handle unclosed ```python blocks (truncation fallback instead of line-by-line `#`-matching).
    4. Re-run with both fixes → **35%**.
  - **Cost**: Phase 3d went from 243s → 652s (2.7× longer due to 2048 vs 512 token budget). Full bench: 1866s (31 min) vs 1386s (23 min).
  - **Tests added** (`tests/test_eval.py::TestLiveCodeBench`, 2 new):
    - `test_extractor_handles_truncated_code_block` — unclosed ```python with reasoning prose before it → code extracted, prose not leaked
    - `test_extractor_base_last_block_handles_truncation` — same coverage on `base.py::_extract_last_code_block`
  - **Task 130(b) amendment**: retraction stands — the original bench run was measuring the wrong prompt. But the CoT hypothesis itself (remove "no explanations"-style directive, invite reasoning) is now EXPERIMENTALLY VALIDATED with the right codepath + budget. The Completed entry for Task 130(b) should note this un-falsification.
  - **Follow-up threads**:
    - (i) **[easy] Fill the Gaps regression**: inspect the saved response to see whether CoT led the model astray or the prompt shape specifically hurt this problem. Cheap to fix if it's prompt-shape.
    - (ii) **Medium/hard still mostly stuck**: 1/7 medium, 0/4 hard passing. Capability ceiling or token-budget ceiling? 2048 may still be short for hard problems with complex reasoning. Try 4096 and measure Phase 3d time + LCB delta.
    - (iii) **Ranking rule A (best-of-N temp>0)** is now the natural next lever — and since CoT is confirmed to help, N different reasoning paths are more likely to include a correct one.
    - (iv) **Test the two-prompts-stay-in-sync contract**: add a unit test asserting `hypercar_bench.phase3d` prompt string matches `LiveCodeBenchBenchmark.format_prompt` (or just consolidate into one function). Prevents future drift.
...
- **Task 130(b)**: LiveCodeBench CoT prompt tuning — enable reasoning, remove "no explanations" directive (2026-04-23)
  ...
  - **✅ UN-FALSIFIED (2026-04-23 via Task 255)**: once Task 254 unified the prompts AND Task 255 bumped the token budget from 512 to 2048 AND hardened the extractor for truncated blocks, LCB moved from **30% → 35%** (+1 PASS, +5 pp). CoT genuinely helps on this model — the only reason the earlier bench looked neutral was that the bench was running a different prompt entirely. Original Task 130(b) hypothesis was correct; the implementation just touched the wrong file.
...
- **2026-04-24 pass 60 CLOSURE**: Pass 60 of the research loop cites both papers. MC-MoE (2410.06270) is ingested into LIT_REVIEW.md Pass 60 as a SHIP candidate with full per-paper entry; ICLR 2025 with Aaronhuang-778/MC-MoE code release. MoQE (2310.02410) is cited **within MC-MoE's entry** as the foundational "expert weights are quantization-robust, non-expert weights are not" finding; MoQE is not separately ingested because its 2310 arxiv date is outside the 2401-2612 corpus window, and its technique is captured via MC-MoE's implementation-ready generalization. Ship/defer decision: **MC-MoE SHIP** (filed as Task 273 below, pass-60 code action), **MoQE REFERENCE-ONLY** (technique-captured via MC-MoE, no separate ship needed). Task 272 complete.

## Research-derived tasks (from LIT_REVIEW.md pass 60, 2026-04-24)

Pass 60 is the first research pass after the Qwen3.6-35B-A3B target pivot (user directive 2026-04-23) and explicitly closes Task 272 — the MC-MoE + MoQE citation request filed with the TQ3 weight-unblock tasks (265-272). Pass 60 split selection 50/50 between Area A (Qwen3.6-specific optimization) and Area B (TQ3 weight unblock support); Area B dominated 3:1 because Task 272 drove the primary requirement. Four papers ingested (MC-MoE 2410.06270, MxMoE 2505.05799, MoPEQ 2509.02512, Ban&Pick 2509.06346). Three follow-on implementation tasks filed here plus one research-only task to track AlphaQ (Liquid AI) once an arxiv ID surfaces.

### 273. Ship MC-MoE PMQ per-expert bit allocation for Qwen3.6-35B-A3B (closes Task 272 ship path)
- **Goal**: 6 (sub-4-bit per-expert average on Qwen3.6-35B-A3B's 256-expert layout frees ~3-5 GB of model-bank memory for KV expansion at 1M context), Goal 2 (2.54-bit average on Mixtral at 3.8% accuracy loss is the MC-MoE headline; projected to Qwen3.6's 256-expert × 9-active layout, this is the right per-expert granularity that uniform TQ3 cannot express)
- **Derived from**: LIT_REVIEW.md Pass 60 / MC-MoE (arXiv:2410.06270, ICLR 2025, Aaronhuang-778/MC-MoE code released). Directly closes Task 272's MC-MoE ship-candidate decision. Composes with Task 270 (per-layer bit ladder) as the per-expert extension Task 270 specifies as future work, and with Task 264 (expert-offload) since dynamic-pruning routing via MC-MoE's ODP reduces the NVMe cold-fetch rate.
- **Change**: Three-phase implementation. Phase 1 — **PMQ calibration harness**: port MC-MoE's per-expert sensitivity-probe from PyTorch to MLX in new `omlx/mc_moe_calibration.py`. Run over Hypercar's code-calibration corpus (Code-Search-Net + code-review set, ~128 samples × 2K tokens). Output: per-(layer, expert) sensitivity score table saved to `bench/snapshots/qwen36_mc_moe_sensitivity.npz`. Phase 2 — **Integer Programming solve**: formulate the bit-width allocation IP (256 experts × 48 layers = 12288 binary decision variables over {2, 3, 4}-bit slots; objective minimize total bits subject to sum-of-expert-sensitivity-weighted-error ≤ quality-budget). Use CBC or SCIP via PuLP (pure-Python IP solver). Output: per-expert bit-width configuration JSON `config/qwen36_mc_moe_bits.json`. Phase 3 — **TurboQuant integration**: extend `omlx/turboquant_convert.py` with `--expert-bits config.json` flag that reads the per-expert bit map and applies bit-width per linear at convert time. The existing TQ3 path assumes uniform bit-width; this flag overrides per (layer, expert) based on the config. (Optional Phase 4 deferred to follow-on task: MC-MoE's Online Dynamic Pruning — modifies runtime router to prune inactive-expert activations; defer until Phase 1-3 validates at quality floor.)
- **Verify**: (a) PMQ calibration completes in < 60 min on M4 Pro for Qwen3.6-35B-A3B-4bit baseline (one-time offline). (b) IP solve completes in < 5 min and produces a valid per-expert bit config (all 12288 experts assigned {2, 3, 4}). (c) Average bit-width across all experts ≤ 3.0 (stronger than the 3.5-bit Task 270 target because MC-MoE's per-expert granularity is finer than per-layer). (d) Model converted with `--expert-bits config/qwen36_mc_moe_bits.json` loads successfully on M4 Pro; Metal peak at load < 14 GB (vs 17.5 GB baseline — ≥3.5 GB reduction). (e) HumanEval pass@1 within 3 pp of baseline-4-bit (quality preservation floor — 3 pp is MC-MoE's headline Mixtral gap; we accept 3 pp here and tighten to 2 pp if calibration allows). (f) NIAH@4K PASS. (g) NIAH@64K PASS on the compressed model (validates that per-expert compression does not degrade long-context retrieval). (h) Compose-test with Task 264 expert-offload: convert once with `--expert-bits` + load with `--moe-hot-cache-size 32 --moe-offload-path /tmp/qwen36_experts.bin` → verify memory reduction stacks multiplicatively (per-expert-bit × hot-cache subset) and quality gates still pass. (i) Compose-test with Task 270 per-layer ladder: if Task 270 has also landed, verify the two configs compose without conflict (per-expert overrides per-layer defaults). (j) Findings note `bench/snapshots/qwen36_mc_moe_findings.md` documenting measured compression ratio, quality delta, and whether the 2.54-bit Mixtral headline is reproducible on Qwen3.6 (likely not — 256-expert layout is flatter than 8-expert — but document the actual Qwen3.6 bit-width average for future reference).
- **Effort**: L (3-4 weeks — Phase 1 calibration 1 week, Phase 2 IP formulation + solve 3 days, Phase 3 TurboQuant integration 1 week, validation + findings 1 week)
- **Depends on**: Task 257 (Qwen3.6 smoke), Task 267 (skip-list/seed cleanup — avoids calibration confounding), Task 266 (depth-sweep informs the initial quality-budget constraint). Soft dependency on Task 270 — MC-MoE fills Task 270's per-expert extension, so if Task 270 ships first with per-layer-only, Task 273 composes above it. Related to Task 264 (expert-offload) — orthogonal but multiplicative benefits.

### 274. Ship Ban&Pick routing optimization on Qwen3.6-35B-A3B (Qwen3-validated transfer)
- **Goal**: 2 (direct Qwen3-30B-A3B validation shows AIME2024 80.67 → 84.66, GPQA-Diamond 65.66 → 68.18 — quality improvements on the A3B family that Qwen3.6 extends; transfer expectation high), Goal 3 (1.25× vLLM inference speedup on Qwen3-30B-A3B — direct decode-speed improvement at no quality cost, training-free and plug-and-play), Goal 6 (router-level pruning via Ban reduces active-expert count per token, shrinks effective working set, frees hot-cache headroom)
- **Derived from**: LIT_REVIEW.md Pass 60 / Ban&Pick (arXiv:2509.06346, OpenReview KWMR2YfC55). First pass-40-60 paper with direct Qwen3-30B-A3B experimental validation. Training-free, plug-and-play — low integration cost. Natural composition with Task 264 (expert-offload) since Ban's expert-pruning reduces cold-fetch rate.
- **Change**: Three-phase implementation. Phase 1 — **key-expert calibration (Pick component)**: run Hypercar's calibration corpus (code + math + reasoning, ~200 samples × 2K tokens) on Qwen3.6-35B-A3B with routing-probability instrumentation in the MoE forward path. For each (layer, expert), accumulate contribution score = (routing probability) × (output-magnitude on calibration samples). Identify the top-K key experts per layer (K ~10-20% of routed experts). Output: key-expert table `config/qwen36_ban_pick_keys.json`. Phase 2 — **layer-token sensitivity calibration (Ban component)**: for each layer, measure per-expert-activation sensitivity on held-out calibration samples — which expert activations can be pruned with ≤1% quality drop. Output: per-layer pruning-eligibility table `config/qwen36_ban_pick_bans.json`. Phase 3 — **router modification**: create `omlx/moe_routing.py` as a plug-in above mlx_lm's MoE router. On forward pass: Pick boosts routing probability for key experts by +10-20% (calibrated); Ban prunes expert activations whose (layer, expert) is in the pruning-eligibility table and whose routing probability falls below a threshold. Wire via CLI flag `--routing-policy banpick` on hypercar_server (default: standard). Fallback: if flag is off, runtime is unchanged (zero-cost when disabled).
- **Verify**: (a) Calibration phases complete in < 45 min on M4 Pro each (two calibrations, one each for Pick and Ban; can share a single calibration pass if forward-hook allows). (b) Model loads with `--routing-policy banpick` and generates coherently on BENCHMARKS.md Run 20 prompts. (c) Goal 2 quality gate: HumanEval pass@1 improves or stays within 1 pp of baseline; Code Intel gate unchanged. (d) Goal 3 decode speed: measured decode throughput at 4K context improves by ≥ 10% vs baseline (direct translation of the 1.25× vLLM result; allow for MLX-specific divergence). (e) NIAH@4K PASS; NIAH@64K PASS (no long-context regression from routing changes). (f) Ablation: Pick-only and Ban-only runs — each should show a component of the combined improvement (Pick for quality, Ban for speed). (g) Compose-test with Task 264 expert-offload: `--routing-policy banpick --moe-hot-cache-size 32` → hot-cache hit rate should improve (Ban prunes cold activations before they fetch) and decode speed should stack Ban's 1.25× with the expert-offload's baseline. (h) Compose-test with Task 273 MC-MoE: verify that PMQ's per-expert bit allocation does not conflict with Pick's boosted key experts (a key expert that MC-MoE assigns 2-bit may need 3-bit if Pick pushes its activation rate up; the two calibrations may need to iterate). (i) Findings note `bench/snapshots/qwen36_ban_pick_findings.md` documenting Qwen3.6 transfer — specifically whether the Qwen3-30B-A3B quality-gain pattern replicates on Qwen3.6-35B-A3B, and what the key-expert set looks like (256 experts × 48 layers is much denser than Qwen3-30B-A3B's expert landscape, so the Pick set may be more diffuse).
- **Effort**: M (2-3 weeks — Phase 1 calibration 3 days, Phase 2 calibration 3 days (can share infrastructure), Phase 3 router-modification implementation 1 week, validation + compose-tests 1 week)
- **Depends on**: Task 257 (Qwen3.6 smoke). Task 262 (MoE re-validation at 256 experts — provides routing statistics that inform the Pick/Ban thresholds). Soft dependency on Task 273 (MC-MoE) — Ban&Pick and MC-MoE's ODP are related but distinct; if both ship, the Pass-60 findings note above is the reconciliation path.

### 275. Ship MoPEQ Hessian-trace per-expert sensitivity scoring for Qwen3.6 VLM (VLM-aware alternative to MC-MoE)
- **Goal**: 6 (4× compression with minimal accuracy loss on MoE-VLM per MoPEQ — Qwen3.6-35B-A3B's vision-encoder + 256-expert MoE architecture is a direct fit), Goal 2 (Hessian-trace sensitivity is structurally better than activation-frequency heuristics for long-tailed expert distributions like Qwen3.6's 256-expert × 3.5%-active layout)
- **Derived from**: LIT_REVIEW.md Pass 60 / MoPEQ (arXiv:2509.02512, IEEE Conference 2025). Only pass-40-60 paper with explicit MoE-VLM per-expert quantization validation (DeepSeek-VL2, MolmoE). Competes with Task 273 (MC-MoE) as the sensitivity-scoring method; may ship as alternative or as regularization layer above MC-MoE's IP.
- **Change**: Three-phase implementation. Phase 1 — **Hessian-trace sensitivity probe**: implement Hutchinson trace estimator for per-expert Hessian in `omlx/mopeq_sensitivity.py`. For each (layer, expert) linear, compute trace(H) approximated as mean over K random Rademacher vectors v of v-transpose H v. Calibration corpus: Hypercar code-calibration corpus + (if Task 261 preserves vision encoder) a small multimodal calibration set. Output: per-(layer, expert) Hessian-trace score table. Phase 2 — **expert clustering**: k-means cluster (k=10-20) over per-expert Hessian-trace vectors; assign bit-width per cluster rather than per individual expert (regularizes against calibration noise on Qwen3.6's 256-expert layout). Output: per-cluster bit-width map `config/qwen36_mopeq_clusters.json`. Phase 3 — **integration**: extend `omlx/turboquant_convert.py` with `--mopeq-clusters config.json` flag, either replacing or composing with Task 273's `--expert-bits` flag. If both flags are set, MoPEQ's clustering acts as a regularizer on MC-MoE's IP solution (constrain IP to assign same bit-width within a cluster); if only MoPEQ, clustering is the sole bit-allocation method.
- **Verify**: (a) Hutchinson trace estimator converges to stable values within 50 Rademacher samples per expert (measure via variance across repeated runs). (b) Calibration phase completes in < 60 min on M4 Pro. (c) Cluster output has sensible structure — high-sensitivity experts cluster together, low-sensitivity experts cluster together (visual inspection of the cluster centroids + ablation to confirm). (d) Model converted with `--mopeq-clusters` loads and generates coherently. (e) HumanEval pass@1 within 3 pp of baseline (Task 273's floor applies). (f) NIAH@4K PASS. (g) A-B test with Task 273: if both Task 273 (MC-MoE) and this task (MoPEQ) ship, compare (a) MC-MoE alone, (b) MoPEQ alone, (c) MC-MoE+MoPEQ-regularized — pick the best-quality-per-memory variant as the production default. (h) If Qwen3.6 Task 261 preserves the vision encoder, also measure vision-language quality post-quantization on a small multimodal eval set (even if Hypercar doesn't ship multimodal features, validate that per-expert quantization doesn't break VLM capability as a form of technical debt control). (i) Findings note comparing MoPEQ's Hessian-trace scoring to MC-MoE's IP-implicit scoring — which produces better sensitivity estimates on Qwen3.6's long-tailed expert distribution.
- **Effort**: M (2-3 weeks — Hutchinson probe 1 week, clustering + integration 1 week, validation + A-B vs MC-MoE 1 week)
- **Depends on**: Task 257 (Qwen3.6 smoke), Task 267 (skip-list/seed cleanup). Soft dependency on Task 273 (MC-MoE) — if MC-MoE ships first, MoPEQ composes as regularizer; if parallel, A-B test picks winner. Task 261 (vision-encoder ablation) provides context for whether VLM-specific validation applies (if vision encoder is removed, MoPEQ's VLM advantage is partial — still gets Hessian-trace scoring, loses VLM-validation provenance).

### 276. Research pass — AlphaQ arxiv tracking + MxMoE hardware-aware codesign for M4 Pro
- **Goal**: 6 (calibration-free bit allocation via AlphaQ would eliminate the PMQ calibration cost in Task 273; MxMoE's hardware-aware codesign would optimize for M4 Pro's Metal throughput specifically)
- **Derived from**: LIT_REVIEW.md Pass 60 / AlphaQ (Liquid AI, March 2026, arxiv ID unavailable at pass-60 search time) and MxMoE (arXiv:2505.05799, OpenReview public). AlphaQ's calibration-free property would simplify Task 273; MxMoE's three-axis codesign (sensitivity × activation × hardware) adds the hardware-aware dimension Task 270 specifies as extensible. Both deferred from ship-list this pass pending (a) AlphaQ arxiv ID surfacing, (b) M4 Pro Metal quantized-matmul throughput table rebuilt from scratch for MxMoE.
- **Change**: Two sub-actions. (a) **AlphaQ tracking**: in pass 61 of the research cron, re-search arxiv for AlphaQ Liquid AI calibration-free MoE bit allocation; if arxiv ID surfaces, add to LIT_REVIEW.md pass 61 with standard per-paper format and file a ship-or-defer decision. (b) **MxMoE hardware profiling (prerequisite for ship)**: build M4 Pro Metal quantized-matmul throughput table via microbenchmarks in `omlx/bench/metal_quant_throughput.py`. Measure tok/s for {2, 3, 4, 8}-bit × {gemv, gemm} × {small, medium, large} matrix sizes representative of Qwen3.6's linear operations. Output: throughput table JSON that MxMoE's optimizer would consume. If the table shows negligible bit-width throughput variation (memory-bandwidth dominated on unified memory), MxMoE's hardware-aware axis collapses to memory-size-only — document as a dead-end-for-M4-Pro and update Task 273 to add the hardware-aware axis as a no-op.
- **Verify**: (a) Pass 61 LIT_REVIEW.md entry for AlphaQ if arxiv ID found; otherwise documented in pass-61 gaps-carried as "AlphaQ arxiv still unavailable, Liquid AI research blog only, defer". (b) Metal quantized-matmul throughput table published to `bench/snapshots/m4pro_quant_throughput.json`. (c) Analysis note: does M4 Pro show meaningful throughput variation across bit-widths? Decision point: if yes (ratio > 1.2× between extremes), MxMoE ship is worthwhile; if no, MxMoE ship is de-prioritized in favor of MC-MoE (Task 273) which is the memory-size-only optimizer. (d) If throughput table says MxMoE is worthwhile, file a follow-on implementation task; if not, update Task 273 findings note to document the M4 Pro specific conclusion and close this thread.
- **Effort**: S-M (1 week — AlphaQ arxiv re-search 0.5 day, Metal throughput microbenchmarks 3 days, analysis note 1 day, follow-on filing if needed 0.5 day)
- **Depends on**: Nothing — research-only + microbenchmark task; can run at any time.

## Research-derived tasks (from LIT_REVIEW.md pass 61, 2026-04-24)

Pass 61 is the second pass in the post-Qwen3.6 pivot arc. It ingests four papers across the tri-axis Area A (Qwen3.6 specifics — Qwen3 Technical Report 2505.09388), Area B (TQ3 weight unblock — DuQuant 2406.01721 NeurIPS 2024, TurboBoA 2602.04929 ICLR 2026), and Area C (held-gap closure — Long-Context Quantization 2505.20276 EMNLP 2025). Pass 61 closes both pass-60 gap-carried items (#1 Qwen3 Tech Report primary-source citation, #2 SWE-Bench / long-context quantization degradation literature). Four follow-on tasks filed here.

### 277. Audit Hypercar Qwen3.6 RoPE/YaRN/DCA config against Qwen3 Technical Report (closes Task 260)
- **Goal**: 1 (native 1M context depends on correct ABF rope_theta=1,000,000 + YaRN schedule + DCA chunked-attention mask; a misconfiguration here silently degrades retrieval quality at long context and explains any Task 260 audit discrepancy)
- **Derived from**: LIT_REVIEW.md Pass 61 / Qwen3 Technical Report (arXiv:2505.09388, Qwen Team / Alibaba Cloud). Direct closure of pass-60 gap-carried item #1. Primary source for Task 260's RoPE audit.
- **Change**: Three-phase audit (read-only, documentation-focused). Phase 1 — **config audit**: verify `omlx/hypercar_server.py` and mlx_lm loader configure Qwen3.6-35B-A3B with `rope_theta=1_000_000` (ABF base frequency per section 2.x of 2505.09388). Check the HF config.json inherits this. Phase 2 — **YaRN activation audit**: trace the chat-template handling in `omlx/hypercar_server.py` to confirm YaRN is activated on long-context queries per the Qwen3 Tech Report specification; if chat template is not preserved through the server, file a fix. Phase 3 — **DCA mask audit**: trace the attention path in `mlx_lm` or Hypercar's patches to verify Dual Chunk Attention mask shape is supported for Qwen3.6; DCA is the inference-time chunked-attention pattern that composes with YaRN to give 4× extension. If mlx_lm's attention does NOT preserve DCA, quantify the impact (is Qwen3.6 still able to run at 1M without DCA, just with quality degradation beyond 256K?) and file fixes.
- **Verify**: (a) Audit note `bench/snapshots/qwen36_rope_audit.md` documenting the three configuration checks with primary-source citations to 2505.09388 sections. (b) If config discrepancies are found, filed as follow-on tasks and RoPE/YaRN/DCA tested on hypercar_bench NIAH@16K + NIAH@64K (if discrepancy is corrected, both should PASS). (c) If no discrepancies, document that Hypercar's config matches the Qwen3 Tech Report specification; this is a clean close-out of Task 260 and a citation anchor for future Qwen3.6-era debugging. (d) Cross-reference: if a Qwen3.6-specific technical report lands (beyond the Qwen3 report), add to the audit as a supplement; note the delta (ABF base frequency, YaRN schedule, DCA interaction) between Qwen3 and Qwen3.6.
- **Effort**: S (1-2 days — paper-read + config-grep + findings note; no code change unless discrepancies are found)
- **Depends on**: Task 260 (Qwen3.6 RoPE audit — this task is the research-input side of that audit). Task 257 (Qwen3.6 smoke — provides the running baseline).

### 278. Ship DuQuant block-diagonal rotation as TQ3 pre-processor (Area B TQ3 unblock, W3 weight path)
- **Goal**: 6 (block-diagonal rotation + zigzag permutation neutralizes "massive outliers" per DuQuant NeurIPS 2024 paper — the specific outlier class that TQ3.5's abandoned Givens variant failed to handle; rotation is computationally lighter than QuaRot's full Hadamard (O(b²) per block vs O(d²) full) and composes with TQ3's WHT as a pre-processor for 3-bit weight quantization), Goal 2 (5% improvement on Commonsense-QA at W4A4 across LLaMA sizes, 10% on Vicuna-v1.5-13B zero-shot MMLU — quality preservation evidence for sub-4-bit weight quantization that Task 270 needs)
- **Derived from**: LIT_REVIEW.md Pass 61 / DuQuant (arXiv:2406.01721, Chen et al., NeurIPS 2024, code released). Provides the W3 weight path Task 270 (per-layer mixed-precision ladder) lacks; direct competitor and composer with QuaRot (Task 271, gated on ablation).
- **Change**: Three-phase implementation. Phase 1 — **rotation construction port**: create `omlx/duquant_rotation.py` porting DuQuant's block-diagonal rotation construction from PyTorch to MLX. The rotation-construction is calibration-time (offline); port the greedy outlier-dimension identification + per-block orthogonal-rotation construction + zigzag-permutation-across-blocks algorithm. Output: per-layer (rotation matrix blocks + zigzag permutation) saved as an auxiliary structure `config/qwen36_duquant_rotations.safetensors`. Phase 2 — **convert-time integration**: extend `omlx/turboquant_convert.py` with `--duquant-preprocess` flag that applies DuQuant rotation + zigzag permutation to weights before TurboQuant's WHT + Beta codebook encoding. The two-stage pipeline: DuQuant neutralizes massive outliers (rotation absorbs the outlier energy into a more uniform distribution), then TurboQuant's WHT + Beta codebook handles the final 3-bit encoding on the now-outlier-neutralized weights. Phase 3 — **inference-time rotation apply**: add a per-layer matmul (rotation apply) to the attention/MLP forward path before quantized weight lookup — note that DuQuant's rotation is applied to weights not activations, so this is a one-time modification at convert time, NOT a per-inference rotation. Clarify in the implementation.
- **Verify**: (a) Rotation construction completes in < 30 min on M4 Pro for Qwen3.6-35B-A3B (offline calibration). (b) Model converted with `--duquant-preprocess` loads and generates coherently on BENCHMARKS.md Run 20 prompts. (c) Quality A-B: convert Qwen3.6-35B-A3B with and without `--duquant-preprocess` at the same bit-width (3-bit weight), measure HumanEval pass@1, MMLU-Pro, Code Intel — DuQuant variant should be within 1 pp or better. (d) NIAH@4K PASS on both; NIAH@64K PASS on both (outlier suppression should help at long context). (e) Measure memory savings: DuQuant's rotation matrices themselves take ~50-100 MB for Qwen3.6; net memory is still a net reduction because the rotated-weights quantize cleaner at 3-bit than un-rotated. (f) Compose-test with Task 273 (MC-MoE): the pipeline `--duquant-preprocess --expert-bits config.json` should work — DuQuant rotates first, MC-MoE's per-expert bits apply second; verify no conflict. (g) Compose-test with Task 271 (QuaRot, if ablation passes): DuQuant's block-diagonal is structurally different from QuaRot's full Hadamard; A-B or combine depending on findings. (h) Findings note `bench/snapshots/qwen36_duquant_findings.md` documenting outlier-suppression effect and compression-vs-quality trade-off on Qwen3.6's 256-expert MoE layout.
- **Effort**: M (2-3 weeks — Phase 1 rotation port 1 week, Phase 2 convert-time integration 3-4 days, Phase 3 sanity-check of the inference path 2 days, validation + A-B testing 1 week)
- **Depends on**: Task 257 (Qwen3.6 smoke), Task 267 (skip-list/seed cleanup — avoids calibration confounding). Soft dependency on Task 271 (QuaRot ablation) — if QuaRot fails, DuQuant is the fallback; if QuaRot passes, A-B or combine. Soft dependency on Task 273 (MC-MoE) — composes as pre-processor before per-expert bit allocation.

### 279. Ship TurboBoA attention-aware batched PTQ backend for TQ3 calibration (Area B TQ3 unblock, calibration speed)
- **Goal**: 6 (3× calibration speedup over BoA + inter-layer dependency accounting — the specific improvement over Hypercar's current per-layer TQ3 calibration path; SOTA at W3 and W4A4 per ICLR 2026 paper), Goal 3 (faster calibration pipeline means per-layer-ladder Task 270 and per-expert MC-MoE Task 273 calibration costs both drop, shortening the iteration loop between quantization configurations)
- **Derived from**: LIT_REVIEW.md Pass 61 / TurboBoA (arXiv:2602.04929, ICLR 2026). Newer faster generation of GPTQ-family that Task 269 (GPTQ) specifies as the generic post-hoc weight-quant method. Three innovations: joint multi-channel quantization (3× speedup), inter-layer error correction, adaptive grid with coordinate descent.
- **Change**: Three-phase implementation. Phase 1 — **paper+code audit**: confirm ICLR 2026 code release status (read paper + check GitHub for author code). If code not released, plan reimplementation from paper description (+1-2 weeks). Phase 2 — **algorithm port**: create `omlx/turboboa_calibration.py` implementing (a) joint multi-out-channel quantization with closed-form error compensation (batched calibration replacing BoA's sequential per-channel loop); (b) correction mechanism — during calibration of layer L, track the output error induced by quantization of layers 1..L-1 and fold it forward into layer L's calibration targets; (c) adaptive grid computation with coordinate descent refinement. Phase 3 — **integration**: extend `omlx/turboquant_convert.py` with `--calibration-backend turboboa` as an alternative to the current implicit calibration; when selected, use TurboBoA's batched + error-corrected + adaptive-grid calibration to determine per-layer quantization parameters, feeding into the final 3-bit codec encode.
- **Verify**: (a) Paper read + code release status documented in `bench/snapshots/turboboa_audit.md` (1 day). (b) Port validation on a toy 125M parameter model first — calibration completes in < 5 min and produces sensible bit-level quantization grids (1-2 days). (c) Port validation on Qwen3.6-35B-A3B — calibration completes in < 6 hours on M4 Pro (budget should be 3× faster than current sequential TQ3 calibration; measure before/after). (d) Quality A-B: convert Qwen3.6 with current TQ3 vs TurboBoA-calibrated TQ3, measure HumanEval pass@1, MMLU-Pro, RULER — TurboBoA variant should improve or match. (e) NIAH@4K PASS; NIAH@64K PASS on TurboBoA variant. (f) Compose-test: full Hypercar pipeline `--duquant-preprocess --calibration-backend turboboa --expert-bits config.json` should work — DuQuant rotates, TurboBoA calibrates, MC-MoE assigns per-expert bits, TQ3 codec encodes to 3-bit. Verify no regression from intermediate stages and measure end-to-end quality delta vs each component alone. (g) Findings note documenting whether the inter-layer correction mechanism matters on Qwen3.6's 48-layer depth (at 48 layers the per-layer error compounding should be measurable; TurboBoA should help more than at smaller depths).
- **Effort**: L (3-4 weeks — Phase 1 audit 1-2 days, Phase 2 port 1-2 weeks, Phase 3 integration 3-4 days, validation + A-B + compose-test 1 week)
- **Depends on**: Task 257 (Qwen3.6 smoke), Task 269 (GPTQ task — TurboBoA is the faster generation; if Task 269 is in progress, TurboBoA replaces it at the calibration-backend slot). Soft dependency on Task 278 (DuQuant) — composes as the rotation pre-processor before TurboBoA's calibration. Soft dependency on Task 273 (MC-MoE) — TurboBoA's per-layer calibration feeds MC-MoE's per-expert bit decisions.

### 280. Long-context quantization degradation eval gate for hypercar_bench (closes Area C SWE-Bench gap)
- **Goal**: 1 × 2 (**the exact Hypercar thesis intersection** — quantization of a 35B MoE at 3-bit weights running at 1M context must preserve Goal-2 quality across the long-context eval class; the EMNLP 2025 paper establishes the 4-bit BNB-nf4 Qwen-2.5 72B as a peer-reviewed reference point; Hypercar at 3-bit Qwen3.6-35B-A3B must demonstrate similar or better robustness, and this gate is how we prove it)
- **Derived from**: LIT_REVIEW.md Pass 61 / Long-Context Quantization Systematic Evaluation (arXiv:2505.20276, EMNLP 2025, molereddy/long-context-quantization). Closes pass-60 gap-carried item #2 (SWE-Bench quantization degradation literature). First systematic evaluation of quantized LLMs on long-context tasks; 9.7K test examples × 5 methods × 5 models.
- **Change**: Three-phase implementation. Phase 1 — **eval subset curation**: read the EMNLP paper + github repo to identify the long-context test examples (source benchmarks likely InfiniteBench, ZeroScrolls, RULER subsets). Select a subset of ~100-500 examples representative of the paper's measurement space (InfiniteBench retrieval + summarization + reasoning; ZeroScrolls long-form). Output: `bench/long_context_quant_eval.jsonl` with examples + expected outputs + task categories. Phase 2 — **gate integration**: add `--long-context-quant-gate` flag to `omlx.bench.hypercar_bench` that runs the curated eval subset against the loaded Hypercar model and reports per-task accuracy. Gate definition: "no long-context task shall drop more than 59% vs baseline fp16 Qwen3.6" (the paper's worst-case 4-bit drop; we accept this as a red line and tighten if measurements show headroom). Phase 3 — **baseline measurement**: run the new gate against the current Hypercar TQ3 configuration to establish the baseline measurement; if any task exceeds the 59% drop threshold, that is a blocking signal for any TQ3-or-more-aggressive quantization config and triggers a root-cause investigation.
- **Verify**: (a) Eval subset curated to `bench/long_context_quant_eval.jsonl` with ≥ 100 examples covering retrieval, summarization, reasoning, and at least one multilingual sample (paper notes non-English degrades more). (b) Gate runs in < 30 min on M4 Pro for the full curated subset — if too slow, reduce to a smaller hot-subset that preserves task diversity. (c) Gate integration passes on current TQ3 configuration — no task drops > 59% vs fp16 baseline. (d) Comparison benchmark: if Qwen-2.5 72B fp16 + BNB-nf4 results are publicly available from the paper's github, measure Hypercar Qwen3.6-35B-A3B TQ3 on the same examples and compare; expected: Hypercar at 3-bit will show some drop but the Qwen family's relative robustness compared to Llama should transfer (i.e., Hypercar's drop should be closer to Qwen-2.5 72B BNB-nf4 than to Llama-3.1 70B BNB-nf4). (e) Compose-test: after Tasks 273 (MC-MoE), 278 (DuQuant), 279 (TurboBoA) land, rerun the gate at each composition level; the full stack should improve quality vs TQ3-alone (that's the whole point of the per-expert + rotation + attention-aware calibration stack). (f) Findings note `bench/snapshots/long_context_quant_findings.md` documenting Hypercar's measured quality-under-quantization profile at long context, with direct comparison to 2505.20276's measurements.
